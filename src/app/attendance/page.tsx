...
export default function AttendancePage(){
  ...
  const submit = ()=>{
    ...
    try{
      window.dispatchEvent(new CustomEvent("jijau_saved",{detail:{message:`✓ Attendance saved for ${cls} on ${date}`, type:"success"}}));
    }catch{}
  };
      ...
      {tab==="Students" ? (
        <div className="space-y-3">
          <div className="card p-4 flex flex-wrap gap-3 items-center justify-between">
            ...
            <button onClick={submit} className="btn-primary">Save Attendance</button>
          </div>
          ...
          {isTeacher && (
            <div className="card p-6 border-l-4 border-l-[#FF6B1A]">
              ...
              <div className="mt-4 flex flex-wrap gap-3">
                <button onClick={()=> { if(user?.facultyId) { markTeacherSelf(user.facultyId, date, "Present"); window.dispatchEvent(new CustomEvent("jijau_saved",{detail:{message:`✓ Marked Present for ${date}`, type:"success"}})); }}} className="btn-primary !bg-[#16A34A] hover:!bg-[#15803D]">✓ Mark Present</button>
                <button onClick={()=> { if(user?.facultyId) { markTeacherSelf(user.facultyId, date, "Absent"); window.dispatchEvent(new CustomEvent("jijau_saved",{detail:{message:`Marked Absent for ${date}`, type:"info"}})); }}} className="btn-secondary !text-[#DC2626]">✕ Mark Absent</button>
          ...
          <div className="card p-6">
            <div className="flex flex-wrap items-center justify-between gap-3">
              ...
              {isAdmin && <button onClick={()=>{
                ...
                setTeacherLocal({});
                window.dispatchEvent(new CustomEvent("jijau_saved",{detail:{message:`✓ Teacher attendance saved for ${date}`, type:"success"}}));
