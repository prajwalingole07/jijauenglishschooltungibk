...
export default function ProfilePage(){
  ...
  const { profilePhotos, updateProfilePhoto } = useStore();
  ...
  useEffect(()=>{
    try{
      ...
      if(raw){ const m=JSON.parse(raw); if(user && m[user.username]) setLocalPhoto(m[user.username]); }
    }catch{}
  },[user]);

  const photo = (user && profilePhotos[user.username]) || localPhoto || null;
  ...
  const onFile=(e:React.ChangeEvent<HTMLInputElement>)=>{
    ...
    r.onload=async ()=>{
      ...
      setLocalPhoto(data);
      if(user){
        await updateProfilePhoto(user.username, data);
