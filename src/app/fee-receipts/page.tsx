...
export default function FeeReceipts(){
  ...
  const handleDirectPrint = async (tx:any)=>{
    ...
    await printReceiptPDF({ tx, student:s, transactions: transactions.filter((t:any)=> t.studentId===tx.studentId), settings });
  };
  const handleDirectDownload = async (tx:any)=>{
    const s=students.find(x=> x.id===tx.studentId);
    await downloadReceiptPDF({ tx, student:s, transactions: transactions.filter((t:any)=> t.studentId===tx.studentId), settings });
  };
