...
import { IconEye, IconReceipt, IconWallet, IconEdit, IconTrash, IconCamera } from "@/components/IOSIcons";
    ...
    <div className="space-y-4">
      <div className="flex flex-wrap items-start justify-between gap-3">
        ...
        <div className="flex gap-2">
          <button onClick={exportCsv} className="btn-secondary glow-hover">Export Ledger</button>
      ...
      <div className="card p-3 flex flex-wrap gap-3 items-center">
        ...
        <PillTabs tabs={["All","Completed","Pending"]} active={filter} onChange={setFilter} />
      </div>

      {filtered.length===0? <Empty title="NO RECORDS FOUND" actionLabel="+ New Enrollment" onAction={()=> setOpen(true)} /> : (
        <>
          {/* Mobile Card View */}
          <div className="md:hidden space-y-3">
            {filtered.map(s=>{
              ...
              const done = bal<=0;
              return (
                <div key={s.id} className="card p-4 bg-white/95 border border-slate-200 shadow-sm rounded-2xl space-y-3">
                  <div className="flex justify-between items-start">
                    <div className="flex items-center gap-3">
                      <div className="w-11 h-11 rounded-full overflow-hidden bg-orange-50 border border-orange-200 grid place-items-center shrink-0">
                        {s.photo ? <img src={s.photo} alt={s.name} className="w-full h-full object-cover" /> : <span className="font-bold text-[#FF6B1A] text-sm">{s.name.charAt(0)}</span>}
                      </div>
                      <div>
                        <div className="font-bold text-slate-800 text-sm">{s.name}</div>
                        <div className="text-xs text-slate-500">Roll #{s.roll} • {s.className}</div>
                      </div>
                    </div>
                    <span className={`badge ${done ? "badge-green" : "badge-red"}`}>{done ? "Completed" : "Pending"}</span>
                  </div>
                  ...
                  <div className="grid grid-cols-3 gap-2 bg-slate-50 p-2.5 rounded-xl text-center text-xs">
                    <div>
                      ...
                      <div className="font-bold text-slate-700">₹{s.totalFees.toLocaleString("en-IN")}</div>
                    </div>
                    <div>
                      <div className="text-[10px] text-emerald-600 font-bold uppercase">Paid</div>
                      <div className="font-bold text-emerald-600">₹{s.feesPaid.toLocaleString("en-IN")}</div>
                    </div>
                    <div>
                      <div className="text-[10px] text-rose-500 font-bold uppercase">Balance</div>
                      <div className="font-bold text-rose-600">₹{bal.toLocaleString("en-IN")}</div>
                    </div>
                  </div>

                  <div className="flex items-center justify-between gap-1.5 pt-1 border-t border-slate-100">
                    <button onClick={() => setDetail(s)} className="flex-1 btn-secondary !py-2 text-xs font-bold text-center">View</button>
                    <button onClick={() => setLedger(s)} className="flex-1 btn-secondary !py-2 text-xs font-bold text-center">Ledger</button>
                    <button onClick={() => setFeeModal(s)} className="flex-1 btn-primary !py-2 text-xs font-bold text-center">Fee</button>
                    <button onClick={() => { setEdit(s); setOpen(true); }} className="w-9 h-9 rounded-xl border border-slate-200 grid place-items-center text-slate-600 hover:bg-slate-50"><IconEdit size={14} /></button>
                    <button onClick={() => { if (confirm(`Delete ${s.name}? This will remove ledger & receipts.`)) deleteStudent(s.id); }} className="w-9 h-9 rounded-xl border border-rose-200 grid place-items-center text-rose-500 hover:bg-rose-50"><IconTrash size={14} /></button>
                  </div>
                </div>
              );
            })}
          ...
          {/* Desktop Table View */}
          <div className="hidden md:block j-table-wrap">
            <table className="j-table">
              <thead><tr><th>Roll #</th><th>Student Name</th><th>Gender</th><th>Parent Contact</th><th>Class</th><th>Category</th><th>Total Fee</th><th>Fee Paid</th><th>Fee Balance</th><th>Status</th><th>Actions</th></tr></thead>
              <tbody>
                {filtered.map(s=>{
                  const bal = s.totalFees - s.feesPaid;
                  const done = bal<=0;
      ...
      {detail && <StudentProfile student={detail} onClose={()=> setDetail(null)} onEdit={()=> {setEdit(detail); setDetail(null); setOpen(true);}} />}
...
function StudentProfile({student,onClose,onEdit}:{student:Student; onClose:()=>void; onEdit:()=>void}){
  ...
  return (
    <div className="modal-backdrop" onClick={onClose}><div className="modal-card max-w-[560px]" onClick={e=> e.stopPropagation()}>
      <div className="p-6">
        ...
        <button onClick={onEdit} className="btn-primary w-full mt-6">Edit Student</button>
      </div>
    </div></div>
  );
}
