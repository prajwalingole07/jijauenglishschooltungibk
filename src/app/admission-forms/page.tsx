...
import { downloadAdmissionPDF, printAdmissionPDF, shareAdmissionPDF } from "@/lib/admissionPdf";
                ...
                <tr key={s.id}>
                  ...
                  <td>
                    <div className="flex items-center gap-1.5">
                      <button onClick={()=> setView(s)} className="btn-secondary !py-1.5 !px-2.5 text-xs font-bold flex items-center gap-1"><IconBook size={12}/> View</button>
                      <button title="Download PDF directly" onClick={()=>{
                        const admNo = `ADM-${s.roll}-${new Date((s as any).admissionDate || s.createdAt).getFullYear()}`;
                        const admDate = (s as any).admissionDate ? new Date((s as any).admissionDate).toLocaleDateString("en-GB") : new Date(s.createdAt).toLocaleDateString("en-GB");
                        downloadAdmissionPDF({ student: s, settings, admissionNo: admNo, admissionDate: admDate });
                      }} className="btn-secondary !py-1.5 !px-2.5 text-xs font-bold text-emerald-700 bg-emerald-50 border-emerald-200 hover:bg-emerald-100 flex items-center gap-1">📥</button>
                      <button title="Share to WhatsApp" onClick={()=>{
                        const admNo = `ADM-${s.roll}-${new Date((s as any).admissionDate || s.createdAt).getFullYear()}`;
                        const admDate = (s as any).admissionDate ? new Date((s as any).admissionDate).toLocaleDateString("en-GB") : new Date(s.createdAt).toLocaleDateString("en-GB");
                        shareAdmissionPDF({ student: s, settings, admissionNo: admNo, admissionDate: admDate });
                      }} className="btn-primary !py-1.5 !px-2.5 text-xs font-bold flex items-center gap-1">💬</button>
                    </div>
                  </td>
                </tr>
              ))}
    ...
    <div className="modal-backdrop" onClick={onClose}>
      <div className="bg-[#F3F4F6] rounded-2xl w-full max-w-[860px] max-h-[92vh] overflow-auto shadow-[0_24px_64px_rgba(0,0,0,0.28)] relative" onClick={e=> e.stopPropagation()}>
        ...
        <div ref={printRef} className="bg-white shadow-lg absolute top-0 left-0" style={{width:"210mm", minHeight:"297mm", padding:"12mm", boxSizing:"border-box", transform:`scale(${effectiveScale})`, transformOrigin:"top left"}}>
         ...
         <div className="p-3.5 flex flex-wrap sm:flex-nowrap gap-2.5 sticky bottom-0 bg-white/95 backdrop-blur-md border-t border-slate-200">
           <button onClick={handleDownload} className="flex-1 btn-primary !py-2.5 text-xs sm:text-sm font-bold flex items-center justify-center gap-1.5 shadow-sm">📥 Download PDF</button>
           <button onClick={()=> shareAdmissionPDF({ student, settings, admissionNo: admNo, admissionDate: admDate })} className="flex-1 bg-[#25D366] hover:bg-[#1EBE5D] text-white font-bold py-2.5 rounded-xl text-xs sm:text-sm flex items-center justify-center gap-1.5 shadow-sm transition">💬 WhatsApp PDF</button>
           <button onClick={handlePrint} className="flex-1 btn-secondary !py-2.5 text-xs sm:text-sm font-bold">Print Form</button>
         </div>
      </div>
    </div>
  );
