...
export default function AdmissionForms(){
  const { students, faculties, settings } = useStore();
  const { user } = useAuth();
  const isTeacher = user?.role === "teacher";
  const teacherClass = isTeacher ? (faculties.find(f=> f.id===user?.facultyId)?.className || "") : "";
  const [q,setQ]=useState("");
  const [view,setView]=useState<any|null>(null);
  const filtered = students.filter(s=> (!isTeacher || s.className===teacherClass) && (!q || `${s.name} ${s.roll} ${s.className} ${s.parentMobile}`.toLowerCase().includes(q.toLowerCase())));

  useEffect(()=>{
    const studentId = new URLSearchParams(window.location.search).get("studentId");
    if(!studentId) return;
    const student = students.find(s=> s.id===studentId);
    if(student) setView(student);
  },[students]);
  ...
  return (
    <div className="space-y-4">
      ...
      {view && <AdmissionFormModal student={view} settings={settings} isTeacher={isTeacher} onClose={()=> setView(null)} />}
    </div>
  );
}

function AdmissionFormModal({student, settings, isTeacher, onClose}:{student:any; settings:any; isTeacher:boolean; onClose:()=>void}){
  ...
  const pdfData = { student, settings, admissionNo: admNo, admissionDate: admDate, includeFees: !isTeacher };
  const handlePrint = ()=> printAdmissionPDF(pdfData);
  const handleDownload = ()=> downloadAdmissionPDF(pdfData);
      ...
      <div className="bg-[#F3F4F6] rounded-2xl w-full max-w-[860px] max-h-[92vh] overflow-auto shadow-[0_24px_64px_rgba(0,0,0,0.28)] relative" onClick={e=> e.stopPropagation()}>
        ...
        <div ref={printRef} className="bg-white shadow-lg absolute top-0 left-0" style={{width:"210mm", minHeight:"297mm", padding:"12mm", boxSizing:"border-box", transform:`scale(${effectiveScale})`, transformOrigin:"top left"}}>
          ...
          <div className="mt-4 bg-[#FFFBF0] border border-orange-100 rounded-xl p-3 text-xs">
            ...
            <div className="text-[#5A4A42] mt-1 leading-relaxed">I hereby declare that the information provided above is true to the best of my knowledge. I agree to abide by the rules and regulations of Jijau English School, Tungi (BK).{!isTeacher && " I understand that fees once paid is non-refundable as per school policy."}</div>
          </div>
         ...
         <div className="p-3 flex gap-2 sticky bottom-0 bg-[#F3F4F6] border-t">
          ...
          <button onClick={async ()=> shareAdmissionPDF(pdfData)} className="flex-1 bg-[#25D366] text-white font-bold py-3 rounded-xl flex items-center justify-center gap-2">💬 WhatsApp PDF</button>
