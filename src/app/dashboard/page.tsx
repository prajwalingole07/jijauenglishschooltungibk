    ...
    <div className="space-y-6">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-[#7A6F68] mt-2">Welcome back, Prajwal (Admin). Here is what&apos;s happening today.</p>
        </div>
...
function TeacherDashboard(){
  ...
  const className = faculty?.className || "Nursery";
  ...
  const [dateStr]=useState(new Date().toLocaleDateString("en-GB"));
  ...
  const saveStudentAtt = ()=>{
    ...
    setLocalAtt({});
    alert("Student attendance saved for "+className);
  };
  ...
  const maxPresent = Math.max(1, ...last7.map(d=> d.present));
  return (
    <div className="space-y-4">
      ...
      {tab==="Self Attendance" && (
        <div className="card p-6 space-y-4">
          ...
          <div className="flex gap-3">
            <button onClick={()=> { if(user?.facultyId) { markTeacherSelf(user.facultyId, todayIso, "Present"); alert("Marked Present"); }}} className="btn-primary !bg-[#16A34A]">✓ Mark Present</button>
            <button onClick={()=> { if(user?.facultyId) { markTeacherSelf(user.facultyId, todayIso, "Absent"); alert("Marked Absent"); }}} className="btn-secondary !text-[#DC2626]">✕ Mark Absent</button>
          </div>
      ...
      {tab==="Homework Hub" && (
        <div className="card p-6">
          ...
          <a href="/homework" className="btn-primary mt-4 inline-block">Go to Homework Tracker</a>
        </div>
      )}
    </div>
  );
