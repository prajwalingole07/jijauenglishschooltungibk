...
export default function Header({ onMenu }:{ onMenu:()=>void }){
  const { settings, profilePhotos } = useStore();
  ...
  useEffect(()=>{
    try{
      ...
      if(user && m[user.username]) setLocalPhoto(m[user.username]);
    }catch{}
  },[user]);

  const photo = (user?.username && profilePhotos[user.username]) || localPhoto || null;
  return (
