...
type Toast = { id: string; message: string; type: "success" | "info" | "error" | "live"; duration?: number };
...
export function ToastProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    return () => {
      window.removeEventListener("jijau_live_edit" as any, onLive);
        ...
        {toasts.map((t) => (
          <div
            ...
            className={`pointer-events-auto px-4 py-3 rounded-xl shadow-[0_8px_24px_rgba(0,0,0,0.12)] border text-sm font-bold flex items-center gap-2 animate-in slide-in-from-top-2 ${
              t.type === "success"
                ...
                : t.type === "live"
                ? "bg-[#FF6B1A] text-white border-[#FF6B1A] shadow-[0_8px_24px_rgba(255,107,26,0.25)]"
                : t.type === "error"
          ...
          >
            <span className="text-base">
              {t.type === "success" ? "✓" : t.type === "live" ? "●" : t.type === "error" ? "✕" : "ℹ"}
            </span>
