...
export function StoreProvider({children}:{children:React.ReactNode}){
  ...
  useEffect(()=>{
    const interval = setInterval(fetchCloud, 2500);
  ...
  },[students, faculties, staff, attendance, teacherAttendance, homework, transactions, salaryRecords, portalAccounts, notices, holidays, settings, classOptions, hydrated, getApiUrl]);

  const api:Store = {
    students, faculties, staff, attendance, teacherAttendance, homework, transactions, salaryRecords, portalAccounts, settings, notices, holidays, classOptions,
    isSyncing,
    serverUrl,
    setServerUrl,
    pingLatency,
