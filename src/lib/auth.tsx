...
export function AuthProvider({children}:{children:React.ReactNode}){
  ...
  const { setProfilePhoto } = useStore();
  ...
  const login = async (username:string, password:string, remember:boolean)=>{
    ...
    // Teacher accounts are stored in Firestore through the deployed API.
    // Keep localStorage as an offline fallback for the same device.
    try{
      const raw = localStorage.getItem("jijau_store_v2");
      let localAccounts: any[] = [];
      if(raw){
        const store = JSON.parse(raw);
        localAccounts = store.portalAccounts || [];
      }
      ...
      try {
        ...
        if (response.ok) {
          ...
          cloudAccounts = cloudStore.portalAccounts || [];
        }
      } catch {
        // Offline login can still use the account cached on this device.
      }

      const accounts = cloudAccounts.length > 0 ? cloudAccounts : localAccounts;
      const acc = accounts.find((p:any)=> p.portalId===username && p.status==="Active");
      if(acc){
        // Portal passwords are currently stored as the existing application hash format.
  ...
  const updateProfilePhoto = (data:string)=>{
    ...
    setProfilePhoto(user.username, data);
    setUser(updated);
  };
