...
export async function shareAdmissionPDF(data: AdmissionData) {
  ...
  await sharePdfDocument(doc, {
    ...
    whatsappFallbackText: `*${data.settings.schoolName}*%0AAdmission Form: ${data.admissionNo}%0AStudent: ${data.student?.name}%0AClass: ${data.student?.className}%0APDF attached - please download`,
  });
}
