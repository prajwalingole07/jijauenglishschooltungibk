...
export async function downloadAdmissionPDF(data: AdmissionData) {
  ...
  // Web: force direct download to Downloads folder
  try {
    const blob = doc.output("blob");
    forceDownload(blob as any, fileName);
  } catch {
    doc.save(fileName);
  }
}
...
export async function shareAdmissionPDF(data: AdmissionData) {
  ...
  const safe = sanitizeName(data.student?.name);
  const fileName = `${safe}_${data.admissionNo}.pdf`;
  const text = `Admission Form ${data.admissionNo} - ${data.student?.name} (${data.student?.className || ""})`;
  // Native: write file + share sheet (WhatsApp directly gets PDF)
  if (isNative()) {
    try {
      const { Filesystem, Directory } = await import("@capacitor/filesystem");
      const { Share } = await import("@capacitor/share");
      const base64 = doc.output("datauristring").split(",")[1];
      // Use Cache so Share can attach file; also try Documents for persistence
      const res = await Filesystem.writeFile({ path: fileName, data: base64, directory: Directory.Cache });
      await Share.share({ title: fileName, text, url: res.uri, dialogTitle: "Share PDF via WhatsApp" });
      return;
    } catch {}
  }
  // Web Share API with files - WhatsApp receives PDF directly
  try {
    const blob = doc.output("blob") as Blob;
    const file = new File([blob], fileName, { type: "application/pdf" });
    const canShareFiles = typeof navigator !== "undefined" && typeof (navigator as any).canShare === "function" && (navigator as any).canShare({ files: [file] });
    if (canShareFiles) {
      await (navigator as any).share({ files: [file], title: fileName, text });
      return;
    }
  } catch {}
  // Fallback: direct download to Downloads + also trigger share if possible
  try {
    const blob = doc.output("blob") as Blob;
  ...
  // Do NOT open wa.me text - user asked for PDF not text. Download is done.
}
