...
const _imageCacheAdm = new Map<string, string>();

async function loadImageAsDataUrl(url: string): Promise<string | null> {
  if (_imageCacheAdm.has(url)) return _imageCacheAdm.get(url)!;
  try {
    ...
    if (res.ok) {
      ...
      _imageCacheAdm.set(url, dataUrl);
      return dataUrl;
    }
  } catch {}
  return null;
...
export async function downloadAdmissionPDF(data: AdmissionData) {
  ...
  // 1. If in native Android wrapper with savePdf bridge
  if (typeof window !== "undefined" && (window as any).Android?.savePdf) {
    try {
      const base64 = doc.output("datauristring").split(",")[1];
      (window as any).Android.savePdf(base64, fileName);
      return;
    } catch {}
  }

  // 2. If Capacitor is available
  if (isNative()) {
    try {
      ...
      try { window.dispatchEvent(new CustomEvent("jijau_saved", { detail: { message: `✓ Saved to Documents: ${fileName}`, type: "success" } })); } catch {}
      return;
    } catch {}
  }

  // 3. Web2App / Android WebView / Mobile & Desktop Browser:
  // Stream via /api/download-pdf with Content-Disposition: attachment so Android DownloadManager catches it directly!
  try {
    const base64 = doc.output("datauristring").split(",")[1];
    ...
    if (res.ok) {
      ...
      try { window.dispatchEvent(new CustomEvent("jijau_saved", { detail: { message: `✓ Downloaded to Phone Storage: ${fileName}`, type: "success" } })); } catch {}
      return;
    }
  } catch {}

  // 4. Client-side blob anchor fallback
  try {
...
export async function shareAdmissionPDF(data: AdmissionData) {
  ...
  // 1. Native Capacitor Share (Android / iOS app container)
  if (isNative()) {
    try {
      const { Filesystem, Directory } = await import("@capacitor/filesystem");
      const { Share } = await import("@capacitor/share");
      ...
      const res = await Filesystem.writeFile({ path: fileName, data: base64, directory: Directory.Cache });
      await Share.share({
        title: `Admission Form ${data.admissionNo}`,
        text: `Admission Form for ${data.student?.name} - ${data.admissionNo}`,
        url: res.uri,
  ...
  // 2. Web Share API Level 2: Directly shares the PDF file to WhatsApp on mobile
  if (typeof navigator !== "undefined" && typeof navigator.canShare === "function" && navigator.canShare({ files: [file] })) {
    try {
      await navigator.share({
        title: `Admission Form - ${data.admissionNo}`,
        text: `Admission Form for ${data.student?.name} - Class ${data.student?.className}\n${data.settings.schoolName || "JIJAU ENGLISH SCHOOL"}`,
        files: [file],
      });
      return;
    } catch (err: any) {
  ...
  // 3. Fallback: Save PDF directly to phone storage and open WhatsApp with pre-filled details
  await downloadAdmissionPDF(data);
  const msg = `*${data.settings.schoolName || "JIJAU ENGLISH SCHOOL"}*\n*Admission Form:* ${data.admissionNo}\n*Student:* ${data.student?.name}\n*Class:* ${data.student?.className}\n\n_PDF downloaded to phone storage - attach from Downloads_`;
