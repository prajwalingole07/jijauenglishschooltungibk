...
import { printPdfDocument, savePdfDocument, sharePdfDocument } from "@/lib/pdfActions";
...
export async function generateAdmissionPDF(data: AdmissionData): Promise<jsPDF> {
  const { student, settings, admissionNo, admissionDate, includeFees = true } = data;
  ...
  // Declaration - bigger
  const declText = includeFees
    ? "I hereby declare that the information provided above is true to the best of my knowledge. I agree to abide by the rules and regulations of Jijau English School, Tungi (BK). I understand that fees once paid is non-refundable as per school policy."
    : "I hereby declare that the information provided above is true to the best of my knowledge. I agree to abide by the rules and regulations of Jijau English School, Tungi (BK).";
  const declH = 19;
  doc.setDrawColor(LIGHT_ORANGE.r, LIGHT_ORANGE.g, LIGHT_ORANGE.b);
  doc.setLineWidth(0.3);
...
export async function downloadAdmissionPDF(data: AdmissionData) {
  ...
  const fileName = `${sanitizeName(data.student?.name)}_${data.admissionNo}.pdf`;
