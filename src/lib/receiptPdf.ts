...
function forceDownloadR(blob: Blob, fileName: string){
...
export async function shareReceiptPDF(data: ReceiptData) {
  ...
  const text = `Fee Receipt ${data.tx.receiptNo} - ${data.student?.name} - Rs. ${Number(data.tx.amount).toLocaleString("en-IN")}`;
  // Native: share PDF file directly (WhatsApp gets file attachment)
  if (isNative()) {
    try {
      const { Filesystem, Directory } = await import("@capacitor/filesystem");
      const { Share } = await import("@capacitor/share");
  ...
  // Web Share API with files - directly shares PDF to WhatsApp
  try {
    const blob = doc.output("blob") as Blob;
    ...
    if (canShareFiles) {
      await (navigator as any).share({ title: fileName, text, files: [file] });
      return;
    }
  } catch {}
  // Fallback: directly download PDF to Downloads folder (do NOT open wa.me text)
  try {
    const blob = doc.output("blob") as Blob;
    forceDownloadR(blob, fileName);
  } catch {
