...
export async function shareReceiptPDF(data: ReceiptData) {
  ...
  await sharePdfDocument(doc, {
    ...
    whatsappFallbackText: `*${data.settings.schoolName}*%0AFee Receipt: ${data.tx.receiptNo}%0AStudent: ${data.student?.name}%0AAmount: Rs. ${Number(data.tx.amount).toLocaleString("en-IN")}%0ADate: ${new Date(data.tx.date).toLocaleDateString("en-GB")}%0A%0A*PDF attached - please download from portal*`,
  });
}
