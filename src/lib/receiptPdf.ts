...
export async function shareReceiptPDF(data: ReceiptData) {
  ...
  if (isNative()) {
    try {
      ...
      const res = await Filesystem.writeFile({ path: fileName, data: base64, directory: Directory.Cache });
      await Share.share({
        title: `Fee Receipt ${data.tx.receiptNo}`,
        text: `Fee Receipt for ${data.student?.name} - Rs. ${Number(data.tx.amount).toLocaleString("en-IN")}`,
        url: res.uri,
  ...
  const msg = `*${data.settings.schoolName || "JIJAU ENGLISH SCHOOL"}*\n*Fee Receipt:* ${data.tx.receiptNo}\n*Student:* ${data.student?.name}\n*Amount:* Rs. ${Number(data.tx.amount).toLocaleString("en-IN")}\n*Date:* ${new Date(data.tx.date).toLocaleDateString("en-GB")}\n\n_PDF downloaded to phone storage - attach from Downloads_`;
